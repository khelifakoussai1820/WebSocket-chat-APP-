...
3. Add it to Vercel and Render as `DATABASE_URL`.
4. Apply migrations:
...
NEXTAUTH_SECRET=your_nextauth_secret
NEXTAUTH_URL=https://gosra-delta.vercel.app
```

The WebSocket server should listen on Render's assigned port:

```js
const port = process.env.PORT || 3001;
```

The deployed WebSocket URL is configured in the frontend through:

```env
NEXT_PUBLIC_WS_URL=wss://your-render-websocket-url
```
